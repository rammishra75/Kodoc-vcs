...
export const useVersionStore = create((set) => ({
  ...
  restoreVersion: async (docId, versionId) => {
    try {
      set({ saving: true, error: null });
      const token = localStorage.getItem("token");
      const commitMessage = `Restore ${new Date().toLocaleString()}`;

      const res = await axios.post(`${API_BASE_URL}/api/documents/${docId}/versions/${versionId}/restore`, { commitMessage }, {
        headers: { Authorization: `Bearer ${token}` }
      });
